...
delete_database_query = """
    TRUNCATE TABLE frescati_data
""" 

#LN: Previous query parts removed:
#     WHERE EXTRACT(YEAR FROM datetime) = 2024
#datetime ==> timestamp
#     AND length BETWEEN 0.0 AND 412.0;
#     AND channel = 2

#LN: Later, We'll need to duplicate the query for the other 3 channels.
...
#LN: Delete database (remove comment)
# LibPQ.execute(conn, delete_database_query)

LibPQ.close(conn)

# Sort the DataFrame by :timestamp (ascending order of rows)
df_sorted = sort(df, :timestamp) #LN: Bytt datetime->timestamp
end_date = DateTime("2016-04-16") #LN: Whats the point of this?
df_sorted = filter(row -> row.timestamp <= end_date, df_sorted) #LN: Bytt datetime->timestamp
# Unstack the sorted DataFrame (this will unstack based on sorted datetime)
udf = unstack(df_sorted, :timestamp, :length, :temperature)


lengths_vector = df.length |> unique |> sort  #LN: removes the repetition in df since it has many iterations of each length within (becuase of channel and time)
times_vector = udf.timestamp #LN: a vector containing all time values.

mat_T = Matrix(udf[:,2:10:end])
plotly()
...
#LN: temp remove:   temperature_matrix = coalesce.(M_T, NaN)
temperature_matrix = rand(5:25,100,100) #LN: Test matrix
...
function kalman_filter(data, process_noise, measurement_noise, A) #IF: A = avvikelse skalär
    ...
    filtered_data = Matrix{Float64}(undef, size(data)...) #IF: matex to store similar results
                                           #LN: CONVERTED TO FLOAT64.
=======


function kalman_filter(data, process_noise, measurement_noise, A) #IF: A = avvikelse skalär
    n_rows, n_columns = size(data) #IF:rows and columns in data is identified. 
    filtered_data = similar(data) #IF: creating matrix with filtered data, same size as the data
>>>>>>> d612cc3714a4e0bb1b9d72de082f797824a67122
    ...
    for t in 1:n_rows
        for j in 1:n_columns
            # Predict Step: Includes trend adjustment #IF: predicting steps by adding errors
            x_pred = A * x_est[j] #IF: predict values in data
            P_pred = A * P_est[j] * A + process_noise #predict error

            # Calculate Kalman Gain
            K = P_pred / (P_pred + measurement_noise[t, j]) #IF: scalar to be used

            # Update Step #IF updated after each for-iteration
            x_est[j] = x_pred + K * (data[t, j] - x_pred) #IF: adjust values to reduce error
            P_est[j] = (1 - K) * P_pred
