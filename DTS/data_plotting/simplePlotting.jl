...
matrix = [1 1 1; 4 2 2; 1 1 1]
function avg_rows(matrix)
    return mean(matrix, dims=1)
end
matrix2 = avg_rows(matrix)
short_mat_T = mat_T[1:100, :] #LN: Removes all other temperature-data other than firt 100 rows (all column remain)
mat_T_average_temp = average_rows(short_mat_T) #LN: averages it into a [1xn] matrix where length(n) = length(length_vector)
#plot(lengths_vector, mat_T_average_temp) #LN: Plots this as an averaged temperture by length plot
