...
# ╔═╡ 06da5470-de26-11ef-3e60-5fa9ded4c6c7
using Markdown, InteractiveUtils, WGLMakie, PlutoUI, Statistics, GeoStats, GeoIO, CSV, DataFrames, HypertextLiteral, Random, Unitful, GeoParquet

# ╔═╡ a78dccdb-dc62-467d-aad3-65437b7570e3
using PlutoUI: FilePicker

# ╔═╡ 16bae09b-067b-434c-9716-e1bd2ff0565f
using PlutoTeachingTools: Columns

# ╔═╡ 36c29650-e7e3-42fe-b82c-034b0f214af3
using Dierckx

# ╔═╡ 5c70aede-91eb-411e-b419-448a208e187c
using ColorSchemes

# ╔═╡ e53c31f8-ef0d-4b5c-a005-4659173b0602
using StatsBase, Plots, KernelDensity

# ╔═╡ 89883472-0f55-4286-be26-9bfbf3d84ac5
import PlutoUI: combine

# ╔═╡ 79e8c964-ea6a-4d99-bb8f-7f0e4dcd4ced
set_theme!(theme_dark())

# ╔═╡ ad13079f-a0f6-4c78-8894-bbe5fa6a0b2c
html"""
<style>
	main {
		margin: 0 auto;
    	...
    	padding-right: max(480px, 5%);
	}
</style>
"""

# ╔═╡ 3277a9d6-ad32-4d6a-bde2-93a0aacf54fc
md"# Boreholes' Deviation Analysis"

# ╔═╡ d4bd8429-e43c-460b-956a-f4876aedb6de
md"## Loading Deviation Measurements files"

# ╔═╡ 41c24815-f00f-4612-997c-2d74370a248a
function pick_n_files(n::Int=2, columns::Int=3)
    strings = string.(1:n)  # Convert numbers to strings
	filep = [Slider(1:10) for i in 1:n]
    rows = ceil(Int, n / columns)  # Calculate number of rows based on columns
...
# ╔═╡ ea404ba1-487b-490b-949f-b7f7d9912b62
@bind file01 FilePicker()

# ╔═╡ 35052a29-7aa8-4d9c-a074-0ceb7bc74583
@bind file02 FilePicker()

# ╔═╡ a0e19a50-1174-414f-9a2d-42fd456b223b
@bind file03 FilePicker()

# ╔═╡ 23365ab9-271c-4df5-96b0-b2f040bd21f7
@bind file04 FilePicker()

# ╔═╡ a491d10f-33ef-445e-9563-5c30a2fc6661
@bind file05 FilePicker()

# ╔═╡ 62e34b25-8915-4c6c-870f-a267ad0aede6
@bind file06 FilePicker()

# ╔═╡ 7ec5aab0-f08f-4adb-9e56-c8f05c8ad148
@bind file07 FilePicker()

# ╔═╡ 2c5c3fe9-b32c-41c7-ac4d-3b16a0df7c12
@bind file08 FilePicker()

# ╔═╡ b33c41a4-a1fe-4b55-abb1-5347e2e97871
@bind file09 FilePicker()

# ╔═╡ b3da2860-7732-43bf-8e68-8229163e0efd
@bind file10 FilePicker()

# ╔═╡ b372e592-660b-416f-b281-ce03b045cf5a
@bind file11 FilePicker()

# ╔═╡ 547961da-e048-4f17-909f-ec0b6f19f729
@bind file12 FilePicker()

# ╔═╡ fd4a27df-ac65-408a-8aaa-2b5a4ac19324
files = [file01; file02; file03; file04; file05; file06; file07; file08; file09; file10; file11; file12]

# ╔═╡ 4897c144-8281-444b-947e-482f5e6e027b
md"## Loading Borehole Geographical positions"

# ╔═╡ d79a94aa-1e4b-44bb-b869-a9c84e1b5177
@bind plan FilePicker()

# ╔═╡ 2f9b759a-a625-430e-ad43-4ab96c70390a
begin
	if plan !== nothing
    	# Define a temporary directory and make sure it exists
    	temp_dir = joinpath(@__DIR__, "temp_files")
    	if !isdir(temp_dir)
        	mkdir(temp_dir)
    	end

    	# Define the full path for the temporary file
    	temp_path = joinpath(temp_dir, plan["name"])

    	# Write the file data to the temporary file
    	open(temp_path, "w") do file
        	write(file, plan["data"])
    	end

    	# Step 3: Load the File with GeoIO
    	loaded_data = GeoIO.load(temp_path);
	end
end

# ╔═╡ ce62962c-6cd6-4781-84f2-c50233c410e2
index_points = loaded_data.id

# ╔═╡ 8503fa64-f852-4618-b738-178ee493b2cd
begin
	i_center = findfirst(x->x==1, index_points)
	ref_x = loaded_data.geometry[i_center].coords.x
	ref_y = loaded_data.geometry[i_center].coords.y
end

# ╔═╡ 045d8c71-af33-452d-b508-3aa213b89151
function get_x(bh)
	i = findfirst(x->x==bh, index_points)
	x = loaded_data.geometry[i].coords.x - ref_x
end

# ╔═╡ 55777fcb-2c60-4f20-85ef-ba5e5f03f203
function get_y(bh)
	i = findfirst(x->x==bh, index_points)
	y = loaded_data.geometry[i].coords.y - ref_y
end

# ╔═╡ e41f7660-4bbb-4327-ac24-820815be86a3
function get_meta_data_raw_dev(file)
	if isnothing(file)
		return DataFrame()
	end
	df = CSV.read(IOBuffer(file["data"]),footerskip=2, header = 17, DataFrame)
end

# ╔═╡ 52189a76-ef72-4ca9-b17f-8a8e0beaf29e
function get_df_raw_dev(file)
	if isnothing(file)
		return DataFrame()
	end
	df = CSV.read(IOBuffer(file["data"]),footerskip=2, header = 17, DataFrame)
	...
	df."East." .*= 1u"m"
	df."Elev." .*= 1u"m"
    df."Temp." = uconvert.(u"°C", (df."Temp." .+ 273.15) .* 1u"K")
	
    # Gravity data
    df."G.Tf." .*= 1u"mGal"  # Adjust to actual gravity units if needed
    df."G.Vec." .*= 1u"m/s^2"  # Example if the vector represents acceleration
	
    # Magnetic data
    df."Mag.Tf." .*= 1u"nT"  # Total magnetic field
    df."Mag.Vec." .*= 1u"nT"  # Magnetic vector magnitude
    df."Mag.Dip." .*= 1u"°"  # Magnetic vector magnitude
	
	index = parse(Int, filter.(isdigit, file["name"]))
	df.BoreholeIndex .= index
	return df
end

# ╔═╡ 4d8bf964-8c59-4efb-953d-549f650a9c54
deviation = reduce((x, y) -> vcat(x, y, cols=:union), get_df_raw_dev.(files))

# ╔═╡ 2ef416f5-f939-49d9-a818-4b84785389a8
begin
	deviation.offset_x .= get_x.(deviation.BoreholeIndex)
	deviation.offset_y .= get_y.(deviation.BoreholeIndex)
	deviation.y = deviation.offset_y .+ deviation."North."
	deviation.x = deviation.offset_x .+ deviation."East."
end

# ╔═╡ 0123185c-f3e9-42a5-b0f7-04145948fe8d
begin
	crs_id = 3857
	crs_ = EPSG{crs_id}
	new_df = DataFrame()
	X = deviation.x .+ ref_x
	Y = deviation.y .+ ref_y
	deviation.Z = deviation."Elev."
	deviation.ID = deviation.BoreholeIndex
	geot = georef(deviation, ("x", "y"), crs=crs_)
end

# ╔═╡ 0a591ad0-74a0-486f-ab41-22159c1c5c79
max_depth = minimum(deviation."Elev.")

# ╔═╡ 23de21ac-9b0e-4eee-9d23-63a009912b9d
@bind depth_plot confirm(PlutoUI.Slider(range(0u"m", max_depth, length=100), default = max_depth))

# ╔═╡ 937918d1-8083-4794-ab0b-0b69ba00ac48
bhind = unique(deviation.BoreholeIndex)

# ╔═╡ f8109b9b-0b65-48af-9779-1e412494143d
@bind display_id confirm(MultiCheckBox(bhind, default = bhind))

# ╔═╡ 6bf46fdd-744f-4f9b-946c-308cda2f76e8
function range_axis(df, depth; axis=:x)
	id_x = df."Elev." .> depth
	valid_x = df[!,axis][id_x]
	return extrema(valid_x)
end

# ╔═╡ 5a692d8e-7aac-405e-b735-e6c4506b4fea
begin 
	r_x = range_axis(deviation, depth_plot, axis=:x)
	r_y = range_axis(deviation, depth_plot, axis=:y)
	r_z = range_axis(deviation, depth_plot, axis=Symbol("Elev."))

	s_x = (r_x[2]-r_x[1]) |> ustrip
	s_y = (r_y[2]-r_y[1]) |> ustrip
	s_z = (r_z[2]-r_z[1]) |> ustrip
	# Adjust the scale
    scale = (s_x, s_y, s_z)
end

# ╔═╡ e8f52308-6fd0-4c7a-86b5-638dada68f10
begin
	grouped_deviation = groupby(deviation, :BoreholeIndex)
	all_Temp = Float64[]
    # Create a 3D plot
    fig = WGLMakie.Figure(size = (800, 600))
    ax = Axis3(fig[1, 1], aspect = scale)

    # Iterate through each group
    for group in grouped_deviation
		if group.BoreholeIndex[1] in display_id
			p_i = Point3f(ustrip(group.x)[1], ustrip(group.y)[1], ustrip(group."Elev.")[1])
	        # Extract X, Y, Z coordinates
			valid_pt = group."Elev." .> depth_plot
	        X = ustrip(group.x)[valid_pt]
	        Y = ustrip(group.y)[valid_pt]
	        Z = ustrip(group."Elev.")[valid_pt]
	        T = ustrip(group."Temp.")[valid_pt]
			append!(all_Temp, T)
	        # Generate the points for the line
	        points = [Point3f(x, y, z) for (x, y, z) in zip(X, Y, Z)]

			#c = group.BoreholeIndex[1] in [1; 10] ? :blue : :green
	        # Plot the line
			WGLMakie.text!(p_i, text = string(group.BoreholeIndex[1]), color=:gray)
		
	        WGLMakie.lines!(ax, points, color=T, colormap = :cool,        # Alternative colormap without dark blues
                            colorrange = (minimum(T), maximum(T)))  # Normalize color range)
		end
    end

    # WGLMakie.scale!(ax.scene, scale)
    Colorbar(fig[1, 2], limits = (minimum(all_Temp), maximum(all_Temp)), colormap = :cool)

	# Enable DataInspector for the figure
	insp = DataInspector(fig)
    # Show the plot
    fig
end

# ╔═╡ 0e28bddf-53fd-45c9-b9ec-3edb227e30a6
begin
    fig2 = WGLMakie.Figure(size = (800, 600))
    ax2 = Axis(fig2[1, 1])
    ...
    for group in grouped_deviation
		...
		if group.BoreholeIndex[1] in display_id
			p_i6 = Point2f(ustrip(group.x)[1], ustrip(group.y)[1])
	        # Extract X, Y, Z coordinates
	        X = ustrip(group.x)
	        Y = ustrip(group.y)
			T = ustrip(group."Temp.")
	        ...
	        # Generate the points for the line
	        points = [Point2f(x, y) for (x, y) in zip(X, Y)]

			WGLMakie.text!(p_i6, text = string(group.BoreholeIndex[1]))
		
	        WGLMakie.lines!(ax2, points, color=T)
		end
    end
	# Enable DataInspector for the figure
	insp6 = DataInspector(fig2)
    # Show the plot
    fig2
end

# ╔═╡ b079a4ab-2fe5-4103-8659-587fd2a5c712
begin
	fig9 = WGLMakie.Figure(size = (800, 600))
    ax9 = Axis(fig9[1, 1], xlabel = "X (m)", ylabel = "Y (m)")
    # Variables to track min/max for axis limits and temperature
    all_x, all_y, all_T = Float64[], Float64[], Float64[]
    ...
    for group in grouped_deviation
        if group.BoreholeIndex[1] in display_id
            ...
            # Collect data for global scaling
            append!(all_x, X)
            append!(all_y, Y)
            append!(all_T, T)
            ...
            points = [Point2f(x, y) for (x, y) in zip(X, Y)]
            WGLMakie.text!(ax9, Point2f(X[1], Y[1]), text = string(group.BoreholeIndex[1]))

            # Apply colormap based on T
            WGLMakie.lines!(ax9, points, color = T,
                            colormap = :cool,        # Better contrast colormap
                            colorrange = (minimum(T), maximum(T)))  # Normalize color range
        end
    end

    # Set axis limits
    WGLMakie.xlims!(ax9, minimum(all_x), maximum(all_x))
    WGLMakie.ylims!(ax9, minimum(all_y), maximum(all_y))

    # Real-world aspect ratio
    ax9.aspect = DataAspect()

    # Add colorbar for reference
    Colorbar(fig9[1, 2], limits = (minimum(all_T), maximum(all_T)), colormap = :cool)

    # Enable DataInspector
    inspecccc = DataInspector(fig9)
...
# ╔═╡ 22069b19-bb1a-4cf2-8478-707393a935e2
md"# Distance Analysis"

# ╔═╡ dbcf7787-41f3-4dad-aee8-c9b4689e4382
md"## Spline fitting for each dimension"

# ╔═╡ bb04b963-3f9a-416e-a14e-0e26a615ddc1
begin
	# Initialize dictionaries to store splines for each borehole
	x_spline = Dict{NamedTuple, Spline1D}()
	y_spline = Dict{NamedTuple, Spline1D}()
	z_spline = Dict{NamedTuple, Spline1D}()
	
	for (key,group) in pairs(grouped_deviation)
		x_spline[key] = Spline1D(ustrip(group."Depth"), ustrip(group.x), k=3)
		y_spline[key] = Spline1D(ustrip(group."Depth"), ustrip(group.y), k=3)
		z_spline[key] = Spline1D(ustrip(group."Depth"), ustrip(group."Elev."), k=3)
	end
end

# ╔═╡ 58362dd0-83b6-44e6-8d39-7ce8e6cc1cde
x_spline[(BoreholeIndex=1,)](ustrip(grouped_deviation[(BoreholeIndex=1,)]."Depth"[end]))

# ╔═╡ 12420769-6268-4df9-b7a8-a5da86766be0
function depth_to_length(bh1::NamedTuple, pos::Float64)	
	return z_spline[bh1](pos)
end

# ╔═╡ 0a327ebc-a8da-410d-bf24-ec39e77e9b99
depth_to_length((BoreholeIndex=10,), 43.08685)

# ╔═╡ 9d18843b-c414-45c9-a24c-9492fb6ecf01
md"## Function to compute the distance between a point of BH1 and each segments of any other BH"

# ╔═╡ 842b04e0-bb4f-4cab-bd0b-fc3331531186
# Function to compute relative distance between any borehole and borehole 1
function relative_distance(depth::Float64, target_borehole::NamedTuple)
    # Borehole 1 key
    bh1_key = (BoreholeIndex=1,)
    # Interpolated coordinates for borehole 1
    x1, y1, z1 = x_spline[bh1_key](depth), y_spline[bh1_key](depth), z_spline[bh1_key](depth)
	
	distances = Float64[]
	for i in 1:length(grouped_deviation[target_borehole]."Depth")
        # Interpolated coordinates for the target borehole at the current depth
        x2, y2, z2 = ustrip(grouped_deviation[target_borehole].x[i]), ustrip(grouped_deviation[target_borehole].y[i]), 			ustrip(grouped_deviation[target_borehole]."Elev."[i])
        
        # Compute Euclidean distance between borehole 1's point and the target borehole's point
...
# ╔═╡ d9a1b255-05cf-48ae-9ca5-2329f7382b6f
begin
	borehole_10_key = (BoreholeIndex=10,)

	distance_15m = relative_distance(15.0, borehole_10_key)
	println("Distance between Borehole 1 and 10 at Depth 15.0 m: $distance_15m m")
end

# ╔═╡ a2331edf-30ec-46c9-a5ac-c9ba86c84691
function twopoints_distance(depth1::Float64, depth2::Float64, target_bh::NamedTuple)
	bh1_key = (BoreholeIndex =1,)
	# Get depth range for borehole 1 and the target borehole
    bh1_depth_min = minimum(ustrip(grouped_deviation[bh1_key]."Depth"))
    bh1_depth_max = maximum(ustrip(grouped_deviation[bh1_key]."Depth"))
    
    target_bh_depth_min = minimum(ustrip(grouped_deviation[target_bh]."Depth"))
    target_bh_depth_max = maximum(ustrip(grouped_deviation[target_bh]."Depth"))
	if depth1 >= bh1_depth_min && depth1 <= bh1_depth_max && depth2 >= target_bh_depth_min && depth2 <= target_bh_depth_max
		x1, y1, z1 = x_spline[bh1_key](depth1), y_spline[bh1_key](depth1), z_spline[bh1_key](depth1)
		x2, y2, z2 = x_spline[target_bh](depth2), y_spline[target_bh](depth2), z_spline[target_bh](depth2)
		dist = sqrt((x2 - x1)^2 + (y2 - y1)^2 + (z2 - z1)^2)
	else
		dist = NaN
		println("Selected depths are out of bounds")
	end
	return dist
end

# ╔═╡ 8be60ed1-6ac3-4820-bcc5-842afa3f24ee
md"## 2D Plot of Distance vs targetBH Depth, for fixed BH1 point"

# ╔═╡ bd42d7f7-4a91-4dc8-b6ee-5dec2770085c
begin
	# Define the fixed depth value in Borehole 1
	depth_value = 15.0
	
	fig3 = Figure()
	ax3 = Axis(fig3[1, 1], xlabel="Depth (m) in Borehole 2", ylabel="Distance (m) from Borehole 1", title="Distance between Borehole 1 (at depth $depth_value) and Borehole 10")
	
	# Define the depth range of borehole 10
	bh10_depths = ustrip(grouped_deviation[borehole_10_key]."Depth")

	distances = relative_distance(depth_value, borehole_10_key)

	# Plot the distances as a continuous function using lines
	lines!(ax3, bh10_depths, distances, color=:blue, linewidth=2)

	# Display the figure
	fig3
end

# ╔═╡ dc699fb6-c4cd-4e19-bcb6-261826acb8f9
begin
	# Define the fixed depth value in Borehole 1
	depth_value1 = 40.0
	target_bh_depth_min = minimum(ustrip(grouped_deviation[(BoreholeIndex = 10,)]."Depth"))
    target_bh_depth_max = maximum(ustrip(grouped_deviation[(BoreholeIndex = 10,)]."Depth"))
	fig5 = Figure()
	ax5 = Axis(fig5[1, 1], xlabel="Depth (m) in Borehole 10", ylabel="Distance (m)", title="Distance between Borehole 1 (at depth $depth_value1) and Borehole 10")

	# Plot the distances as a continuous function using lines
	lines!(ax5, target_bh_depth_min..target_bh_depth_max, t -> twopoints_distance(depth_value1, t, (BoreholeIndex = 10,)), color=:blue, linewidth=2)

	# Display the figure
	fig5
end

# ╔═╡ cc4409b4-39cc-4f84-aefd-fc6deafbe62f
md"## Function to return the elevation given the depth"

# ╔═╡ c140a3a0-dca5-49d0-b6a5-e59214618f8d
# Function to get the elevation at a given depth from a specific borehole
function get_elevation(depth::Float64, borehole_key::NamedTuple)
    # Interpolated coordinates for the target borehole
    z = z_spline[borehole_key](depth)  # This is the elevation
    return z
end

# ╔═╡ 9ff080a4-5753-4b38-bd04-9459bd9c5531
md"# Distance Distribution"

# ╔═╡ e42083d3-b31b-4aae-a78c-e285b91fc800
begin
	depth_range_bh1 = range(0.0, ustrip(grouped_deviation[(BoreholeIndex = 1,)]."Depth"[end]), length = 200)
	depth_range_bh10 = range(0.0, ustrip(grouped_deviation[(BoreholeIndex = 10,)]."Depth"[end]), length = 700)
end

# ╔═╡ 4a1bed7b-083d-4b2b-a32e-27d0b5df1145
begin
	# Generate distance data
	dist_matrix = fill(NaN, length(depth_range_bh1), length(depth_range_bh10))
	for (i,d1) in enumerate(depth_range_bh1)
		for (j,d2) in enumerate(depth_range_bh10)
    		dist = twopoints_distance(d1, d2, (BoreholeIndex=10,))  # Adjust target borehole index
    		if !isnan(dist)
...
# ╔═╡ 583e703e-0102-4f19-ab00-76f78b13c433
begin
	fig6 = Figure(size=(800, 600))
	ax6 = Axis(fig6[1,1], xlabel="Depth Borehole 10", ylabel="Depth Borehole 1", title="Distance Matrix Heatmap")

	# Convert distances to a heatmap
	hm = WGLMakie.heatmap!(ax6, 1:length(depth_range_bh10), 1:length(depth_range_bh1), dist_matrix;
              	colormap=:viridis, colorrange=(minimum(skipmissing(dist_matrix)), maximum(skipmissing(dist_matrix))))

	Colorbar(fig6[1,2], hm)
	fig6  # Display the figure
end

# ╔═╡ 6d2b7c5c-8753-4c2e-9997-11d071321fde
# 4. Create histogram and KDE
function plot_distance_distribution(distances, bins=60)
	# Determine the minimum and maximum distance
    min_dist = minimum(distances)
    max_dist = maximum(distances)
    
    # Create bins starting from min_dist
    bin_edges = range(min_dist, stop=max_dist, length=bins+1)
    # Create a histogram
    Plots.histogram(distances, bins=bin_edges, xlabel="Distance", ylabel="Frequency", title="Distance Distribution", normalize=true, label="Histogram")
end

# ╔═╡ b7f5327f-5b59-4b3e-934f-86fa34727fea
plot_distance_distribution([twopoints_distance(100.0, d, (BoreholeIndex = 10,)) for d in depth_range_bh10])

# ╔═╡ f7b5e7ee-0a58-484e-bbf3-284550f2fbe7
minimum([twopoints_distance(100.0, d, (BoreholeIndex = 10,)) for d in depth_range_bh10])

# ╔═╡ 28feeebb-e2a7-4bf5-9663-e740906f2926
maximum([twopoints_distance(40.0, d, (BoreholeIndex = 10,)) for d in depth_range_bh10])

# ╔═╡ 6f07139c-b224-4f8b-b15c-f9d1068c901c
begin
	selected_distances = [twopoints_distance(40.0, d, (BoreholeIndex = 10,)) for d in depth_range_bh10]  # Extract the row
	fig7 = Figure(size=(800, 400))
	ax7 = Axis(fig7[1,1], xlabel="Distance", ylabel="Frequency", title="Distance Distribution for Fixed Borehole 1 Depth")

	# Create histogram of distances
	hist!(ax7, collect(skipmissing(selected_distances)), bins=60, color=:blue)

	fig7  # Display the figure
end
...
manifest_format = "2.0"
project_hash = "53d2b639dc2ad42310427fb12d2ab2277df72cd8"
...
git-tree-sha1 = "fed736f547e043a25b15fbb59b3cd1348b235376"
...
deps = ["Dates"]
git-tree-sha1 = "313cc77198310b0ff31d07dbf745d5a5eb7cb541"
...
git-tree-sha1 = "572dbf34f41c28b780edc945cc408db21fc752ff"
uuid = "a7073274-a066-55f0-b90d-d619367d196c"
version = "302.1000.100+0"
...
git-tree-sha1 = "4ab907e8579e9cceed8fdff3c4a2dc3823209e14"
...
git-tree-sha1 = "4ed7df1f5eb4a011af60140deabf8b6b5c430c1b"
...
git-tree-sha1 = "2d81bc0a6ab6a4f4d7ec47a6f18ff357fac586c5"
uuid = "e99870d8-ce00-4fdd-aeee-e09192881159"
version = "0.3.0"

    [deps.GeoParquet.extensions]
    QuackIOExt = ["QuackIO", "DuckDB_jll"]

    [deps.GeoParquet.weakdeps]
    DuckDB_jll = "2cbbab25-fc8b-58cf-88d4-687a02676033"
    QuackIO = "7b8617ff-c43a-4c37-a28b-ad5a1791f8ae"

[[deps.GeoStats]]
deps = ["CategoricalArrays", "Chain", "CoDa", "CoordGridTransforms", "CoordRefSystems", "DataScienceTraits", "Dates", "DensityRatioEstimation", "Distances", "GeoStatsBase", "GeoStatsFunctions", "GeoStatsModels", "GeoStatsProcesses", "GeoStatsTransforms", "GeoStatsValidation", "GeoTables", "LossFunctions", "Meshes", "Reexport", "Rotations", "Statistics", "StatsLearnModels", "TableTransforms", "Tables", "Unitful"]
git-tree-sha1 = "2593da59ce02058ec7cfeb64cb46c3b6a105c24c"
...
git-tree-sha1 = "fcfbce45628e72ebe2b8b29b59405afcf926e82f"
...
deps = ["Bessels", "CategoricalArrays", "DataScienceTraits", "Distances", "GeoTables", "LinearAlgebra", "Meshes", "NearestNeighbors", "OhMyThreads", "Optim", "PrecompileTools", "Printf", "Random", "Setfield", "StaticArrays", "Statistics", "TableTransforms", "Tables", "Unitful"]
git-tree-sha1 = "043c5386b1058e6e185d6af4207a586333143afd"
...
git-tree-sha1 = "7e05515160fe0f127d11f924468667aedc8e2ae6"
uuid = "ad987403-13c5-47b5-afee-0a48f6ac4f12"
version = "0.8.2"

[[deps.GeoStatsProcesses]]
deps = ["Bessels", "CpuId", "Distances", "Distributed", "Distributions", "FFTW", "GeoStatsFunctions", "GeoStatsModels", "GeoTables", "LinearAlgebra", "Meshes", "ProgressMeter", "Random", "Statistics", "Tables", "Unitful"]
git-tree-sha1 = "99ed5b9ffd15a1371f224c60feec235928930689"
...
git-tree-sha1 = "426f95ea460e876f25031f04886f9b225736c487"
...
git-tree-sha1 = "2a9a9ab205cb1f0fb64e3be3f45324db7bb04774"
...
git-tree-sha1 = "026b1c28e95e50612550c1d6e0ca89e495be9567"
...
git-tree-sha1 = "b611d6ffff81cde2ff972ba2d880c3d0b9720192"
...
git-tree-sha1 = "9a4b57a775f730b9844c541ee814714b11b77bc5"
...
git-tree-sha1 = "b2d91b5368331e8c1d56a3159c95d7545236ff08"
...
git-tree-sha1 = "335787a3db4e00160dbb854070e29e3b2bc132e9"
...
git-tree-sha1 = "79ce7b25fd94815514c9fca7265461bd0c679643"
...
git-tree-sha1 = "73c83e60acd25f48573deea39cb41b7a5632fd0c"
...
git-tree-sha1 = "8baee70f970d36d4e205094dd5c4a0cab030aa37"
