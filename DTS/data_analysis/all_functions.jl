...
function spline_generator(query::String, conn_string::String, start_date::DateTime, end_date::DateTime, filepath::String, lim::Float64, A::Float64, process_noise::Float64, s::Int64)
    ...
    return spline_2d, fiber_lengths, time_values, mean_absolute_error, max_absolute_error
end
...
spline_2d, fiber_lengths, time_values, mean_absolute_error, max_absolute_error = spline_generator(query, conn_string, start_date, end_date, filepath_channel, first_valid_depth, A, process_noise, smoothness)
full_time_values = collect(0:600:time_values[end])
kalman_BH10, fiber_lengths2, time_values2 = kalman_generator(query, conn_string, start_date, end_date, filepath_channel, first_valid_depth, A, process_noise)
...
T = [spline_2d(x, y)*valid_times(x, boundaries) for x in full_time_values, y in fiber_lengths]  
l2_ = collect(range(fiber_lengths[30], fiber_lengths[end-2], length = length(fiber_lengths[30:end-2])))
L2_ = l2_ .- l2_[1]
...
selected_labels = [string(Date(start_date + Second(val))) for val in selected_ticks]
surfacepl = surface(full_time_values[1:50:end], L2_, T[1:50:end,30:end-2]', 
...
last_day = "2022-12-31"
AirT_dict = Dict(k => v for (k, v) in complete_AirT if Date(first_day) <= Date(k) && Date(last_day) >= Date(k))
